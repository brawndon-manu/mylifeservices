...
import PortalBrand from "./_components/PortalBrand";
import frame from "./_components/PortalFrame.module.css";
import { signOut } from "@/auth";
import { getCurrentUser } from "@/lib/current-user";
import { isElevated, isIT, isAdminUp, canEnterAdmin, ROLE_LABELS } from "@/lib/roles";
import { prisma } from "@/lib/prisma";
import PreviewBar from "./_components/PreviewBar";
import PortalNavigation, { PortalNavigationToggle } from "./_components/PortalNavigation";
...
export default async function PortalLayout({ children }) {
  ...
  return (
    ...
    <div data-portal-chrome="" className={`portal-shell no-focus-zoom ${frame.frame}`}>
      ...
      <PortalNavigation>
        <div className={`portal-chrome-tone ${frame.header} sticky top-0 z-30 border-b lg:hidden`}>
        ...
        <PortalSidebar
          ...
          name={preferredName(user) || user?.email}
          image={user?.image}
        ...
        <div className={`${frame.content} pb-[calc(env(safe-area-inset-bottom)+5.5rem)] lg:pb-0`}>
