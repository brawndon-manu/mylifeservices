...
// rows the screen shows. One truth, two renderings: nothing here knows whether
// it is feeding JSX or a PDF.
//
...
export async function buildAudit(id) {
  ...
  // clock export and the notes spell them plain, so the shift's key ("acuna
  // jose") misses the authorization's ("acuna angel jose"). Where the shift's
  // tokens sit inside EXACTLY ONE authorization key, that key is the person;
  // an ambiguous subset stays unmatched rather than guessing.
  if (hasAuthorizations) {
    const authKeyList = Object.keys(authorized);
    const aliasCache = new Map();
    for (const r of rows) {
      if (!r.authKey || authorized[r.authKey]) continue;
      if (!aliasCache.has(r.authKey)) {
        const tokens = r.authKey.split(" ");
        const hits = authKeyList.filter((k) => {
          const kt = new Set(k.split(" "));
          return tokens.every((t) => kt.has(t));
        });
        aliasCache.set(r.authKey, hits.length === 1 ? hits[0] : null);
      }
      const alias = aliasCache.get(r.authKey);
