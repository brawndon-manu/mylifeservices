...
// pdfjs is loaded lazily (and its node shims installed) by the shared loader,
// so importing this module from a client bundle or an edge route never drags
// the whole PDF stack in. The schedule reader behind the client attestations
// loads it exactly the same way - see src/lib/pdf-globals.js.
//
...
// the overtime split, moved out so a browser can import it - see overtime.js
import { OT_RULES, applyOvertime } from "./overtime.js";
...
export const RULES = {
  ...
  // into a wage claim.
  // the overtime thresholds live in overtime.js now, spread in so this table
  // still reads whole and there is still only one definition of each
  ...OT_RULES,
...
// THE OVERTIME SPLIT MOVED to ./overtime.js on 2026-09-17 - same code, a file
// a browser can import, so the employee's review summary can show what a day
// they have reported does to their overtime. Re-exported here because that is
// where every caller has always read it from.
export { applyOvertime };
